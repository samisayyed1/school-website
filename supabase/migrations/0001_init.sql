...
create table if not exists profiles (
  ...
  email text,                   -- mirror of auth.users.email (login identity)
  phone text,                   -- mirror of auth.users.phone (login identity)
  full_name text not null,
  role user_role not null,
  avatar_url text,
  workspace_email text,         -- Google Workspace email if provisioned
  notification_email text,      -- where the school sends emails (admin-editable)
  notification_phone text,      -- where the school sends WhatsApp (admin-editable)
...
-- Defensive: drop the previous-iteration helper (named current_role, which collided
-- with the reserved PG keyword). Safe to re-run; no-op if it doesn't exist.
...
begin
  ...
  values (
    ...
    -- Default notification channels to the login channels; admin can change later.
    coalesce(new.raw_user_meta_data->>'notification_email', new.email),
    coalesce(new.raw_user_meta_data->>'notification_phone', new.raw_user_meta_data->>'phone')
  )
...
-- Column-level guard: admin (IT) can only change notification channels on other users.
-- Login email/phone and role are protected. Principal/coordinator are unrestricted.
create or replace function public.enforce_admin_profile_scope()
returns trigger language plpgsql security definer as $$
declare
  updater_role user_role;
begin
  -- No auth context (service_role / triggers from auth.users) — allow.
  ...
  if updater_role = 'admin' then
    if old.email           is distinct from new.email
       ...
       or old.workspace_email is distinct from new.workspace_email then
      raise exception 'Admin (IT) may only change notification_email / notification_phone / avatar_url on other profiles';
    end if;
  end if;
...
drop trigger if exists profiles_admin_scope on profiles;
create trigger profiles_admin_scope
  before update on profiles
  for each row execute function enforce_admin_profile_scope();
...
create policy "profiles: staff read all" on profiles for select
  using (app_user_role() in ('admin','principal','coordinator','teacher'));
...
create policy "profiles: admin/principal write" on profiles for all
  using (app_user_role() in ('admin','principal'))
  with check (app_user_role() in ('admin','principal'));
...
create policy "classes: staff write" on classes for all
  using (app_user_role() in ('admin','principal','coordinator'))
  with check (app_user_role() in ('admin','principal','coordinator'));
...
create policy "students: teacher read own classes" on students for select
  using (app_user_role() = 'teacher' and teaches_student(profile_id));
...
create policy "attendance: teacher read own classes" on attendance for select
  using (app_user_role() = 'teacher' and teaches_student(student_id));
...
create policy "attendance: teacher write own classes" on attendance for all
  using (app_user_role() = 'teacher' and teaches_student(student_id))
  with check (app_user_role() = 'teacher' and teaches_student(student_id));
...
create policy "attendance: coordinator/principal write" on attendance for all
  using (app_user_role() in ('principal','coordinator'))
  with check (app_user_role() in ('principal','coordinator'));
...
create policy "exams: teacher write own classes" on exams for all
  using (app_user_role() = 'teacher' and teaches_class(class_id))
  with check (app_user_role() = 'teacher' and teaches_class(class_id));
...
create policy "results: senior staff read all" on results for select
  using (app_user_role() in ('principal','coordinator'));   -- admin (IT) has NO grade access
...
create policy "announcements: read targeted" on announcements for select to authenticated using (
  (target_role is null or target_role = app_user_role())
  and (
    class_id is null
    or app_user_role() in ('admin','principal','coordinator')
    or (app_user_role() = 'teacher' and teaches_class(class_id))
    or (app_user_role() = 'student' and exists (select 1 from students s where s.profile_id = auth.uid() and s.class_id = announcements.class_id))
    or (app_user_role() = 'parent' and exists (select 1 from students s where s.parent_id = auth.uid() and s.class_id = announcements.class_id))
  )
);
...
create policy "announcements: teacher write own classes" on announcements for all
  using (app_user_role() = 'teacher' and (class_id is null or teaches_class(class_id)))
  with check (app_user_role() = 'teacher' and (class_id is null or teaches_class(class_id)));
